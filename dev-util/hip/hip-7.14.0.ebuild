...
if [[ ${PV} == 9999 ]]; then
	...
	SRC_URI="
		https://github.com/ROCm/rocm-systems/releases/download/therock-${PV%.*}/clr.tar.gz -> rocm-clr-${PV}.tar.gz
		https://github.com/ROCm/rocm-systems/releases/download/therock-${PV%.*}/${PN}.tar.gz -> ${P}.tar.gz
		test? (
			https://github.com/ROCm/rocm-systems/releases/download/therock-${PV%.*}/hip-tests.tar.gz -> hip-tests-${PV}.tar.gz
		)
	"
	S="${WORKDIR}/clr"
...
src_unpack() {
	...
	for a in ${A}; do
		case "${a}" in
			rocm-clr-*|clr.tar.gz)
				mkdir "${WORKDIR}/clr" || die
				cd "${WORKDIR}/clr" || die
				unpack "${a}"
				;;
			${P}.tar.gz|hip-*)
				mkdir "${WORKDIR}/hip" || die
				cd "${WORKDIR}/hip" || die
				unpack "${a}"
				;;
			hip-tests-*)
				mkdir "${WORKDIR}/hip-tests" || die
				cd "${WORKDIR}/hip-tests" || die
				unpack "${a}"
				;;
			*)
				cd "${WORKDIR}" || die
